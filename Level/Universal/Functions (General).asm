...
InitControllers:
	...
	bra.w	L_startZ80			; Start the Z80
...
; -------------------------------------------------------------------------
; unused variant using queue #2

PlayFMSound2:
    move.b  d0,fmSndQueue2.w  ; unused
    rts
...
UpdateFMQueues:
    ...
    tst.b   fmSndQueue1.w
    beq.s   .CheckQueue2
    move.b  fmSndQueue1.w,Z80RAM+$1C09.l
    move.b  #0,fmSndQueue1.w
    bra.s   .End
.CheckQueue2:
    tst.b   fmSndQueue2.w
    beq.s   .End
    move.b  fmSndQueue2.w,Z80RAM+$1C09.l
...
DecompPLCFast_Large:
	move.w	#9,plcProcTileCnt.w		; Decompress 9 tiles in this batch
                                    ; ~ Half the amount of the final game
	moveq	#0,d0				    ; Get VRAM address
	move.w	plcBuffer+4.w,d0
	addi.w	#9*$20,plcBuffer+4.w	; Advance VRAM address
...
; -------------------------------------------------------------------------
; ~ This code is bugged in the final, check against it.
DecompPLC_Pop:
	lea	plcBuffer.w,a0
	moveq	#$15,d0
...
.End:
	rts
			...
			INCLUDE	"_Include/Kosinski Plus Decompression.asm"
